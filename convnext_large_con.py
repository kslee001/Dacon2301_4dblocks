...
class MyModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = create_optimizer_v2(
            self.model, "madgradw", lr=LEARNING_RATE, weight_decay=0.01
        )
...
def main():
        ...
        for f in L:
            if ('epoch' in f):
                ...
                if cur_epoch >max_epoch:
                    ...
                    load_target = f
        if max_epoch != 0:
            module = MyModule.load_from_checkpoint(f"{folder_name}/{load_target}")
            print(f"-- checkpoint loaded, starting epoch: {max_epoch}")
    else:
        exit()

    checkpoints = ModelCheckpoint(dirpath=folder_name+'con1', monitor="val_loss", mode="min")
    ...
    trainer = pl.Trainer(
        ...
        logger=WandbLogger(name=f"{model_name}_{now}_{SEED}_con1", project="4dblock"),
        callbacks=callbacks,
        # callbacks=None,
        max_epochs=EPOCHS,
        precision=16,
