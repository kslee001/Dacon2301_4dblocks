...
# model_name = 'swin_large_patch4_window12_384' # 104.71	119.65
...
MODEL_PATH = f"/home/gyuseonglee/workspace/play/checkpoints/{model_name}_{SEED}"
MODEL_PATH = f"{MODEL_PATH}/{os.listdir(MODEL_PATH)[0]}"
...
def train_transform(size, mean, std):
    return T.Compose(
        [
            ...
            T.RandomResizedCrop(size=size, interpolation=T.InterpolationMode.BICUBIC),
...
class BlockDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == "fit":
            ...
            self.train_dataset = BlockDataset(train_df, self.train_transform)
...
def main():
    ...
    if best_model_path :#and os.path.isfile(best_model_path):
        trainer = pl.Trainer(
            ...
            logger=WandbLogger(name=f"{model_name}_{now}_{SEED}", project="inference"),
            # callbacks=callbacks,
            # callbacks=None,
            max_epochs=EPOCHS,
            precision=32,
        ...
        pd.DataFrame(prob).to_csv(f"{SEED}_prob.csv", index=False)
        print("-- prob exported")
